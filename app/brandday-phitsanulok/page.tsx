...
export default function RayongDisplayBoard() {
  ...
  const fetchParticipants = async () => {
    try {
      const res = await fetch("/api/get-phitsanulok-participants");
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "events", "phitsanulok-lucky-draw"), (docSnap) => {
      if (docSnap.exists()) {
    ...
    <main className="min-h-screen bg-gray-900 flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 p-10 text-center mt-10">
        ...
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-white drop-shadow-2xl">
          iHAVECPU<span className={isGrandPrize ? 'text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.6)]' : 'text-red-500'}>BRAND DAY</span>
        </h1>
